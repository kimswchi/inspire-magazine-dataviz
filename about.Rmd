...
timeline <- ggplot() + 
  ...
  #geom_text(aes(x = Date, y = Disp, label = Event), data = filter(timeline, CT == 1),
  #          vjust = -0.2, size = 3.2) +
  #scale_color_manual(values = c("#DD5044", "#1AA260", "#FFCE43")) + 
...
#ptimeline$x$data[[6]]$text <- "look I'm blue"
...
#cleanup
ptimeline$x$data[[9]]$text <- ""
ptimeline$x$data[[9]]$hovertext <- ""

ptimeline

ptimeline$x$data$hovertext
ptimeline$x$data[[1]]$text
```
