...
timeline <- ggplot() + 
  geom_segment(aes(x = Date, y = Disp, xend = Date), data = timeline2, yend = 0,
               size = 0.08, alpha = 0.5) + 
  ...
  geom_point(aes(x = Date, y = Disp, color = Type), data = timeline2, size = 2) +
  scale_color_manual(values = c("#CC2810", "#0000FF", "#4B4B4B")) + 
...
ptimeline$x$layout$legend$x <- c(0.05)
ptimeline$x$layout$legend$y <- c(0.98)
